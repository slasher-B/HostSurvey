...
func FaviconMd5(Url string, timeout int, Path string, proxy string) string {
	client := &http.Client{
...
func DefaultRequests(Url string, timeout int,proxy string) []RespLab {
	...
	if len(regexp.MustCompile("30").FindAllStringIndex(response_status_code, -1)) == 1 {
		...
		if len(regexp.MustCompile("30").FindAllStringIndex(response_status_code_two, -1)) == 1 {
			...
			faviconmd5 := FaviconMd5(Url, timeout, "", proxy)
			RespData := []RespLab{
...
func CustomRequests(Url string, timeout int, Method string, Path string, Header []string, Body string, proxy string) []RespLab {
	var resp_title string
