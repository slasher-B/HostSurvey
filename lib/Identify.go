...
func Identify(url string, timeout int, proxy string) []IdentifyResult {
	...
	for _, resp := range DefaultRequests(url, timeout, proxy) { // Default Request
	...
	for _, rule := range config.RuleData {
		if rule.Http.ReqMethod != "" { // Custom Request Result
			for _, resp := range CustomRequests(url, timeout, rule.Http.ReqMethod, rule.Http.ReqPath, rule.Http.ReqHeader, rule.Http.ReqBody, proxy) {
